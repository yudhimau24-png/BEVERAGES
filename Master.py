...
# Import dotenv secara aman agar tidak crash di Streamlit Cloud
try:
...
# DETEKSI AMAN DAN OTOMATIS UNTUK API KEY
def get_auto_api_key() -> str:
    # 1. Coba ambil dari st.secrets (Streamlit Cloud / .streamlit/secrets.toml)
    try:
    ...
    # 2. Coba ambil dari Environment Variable (.env / server env)
...
if btn_process:
    if not api_key:
        st.error(
            "❌ API Key tidak terdeteksi. Silakan atur file .env / Secrets atau"
            " masukkan manual di sidebar!"
        )
    else:
